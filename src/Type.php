...
abstract class Type implements Stringable
{
    ...
    private static function identify(string $name, bool $allowsNull = false): self
    {
        ...
        $type = match (strtolower($name)) {
            ...
            'enum' => $allowsNull ? new UnionType(new EnumType(), new NullType()) : new EnumType(),
            'datetime' => $allowsNull ? new UnionType(new DateType($name), new NullType()) : new DateType($name),
            'datetimezone' => $allowsNull ? new UnionType(new DateTimeZoneType($name), new NullType()) : new DateTimeZoneType($name),
